...
def get_context(context):
    context.current_state = (
        frappe.form_dict.state if frappe.form_dict.state != "All" else None
    )
    context.current_category = (
        frappe.form_dict.category if frappe.form_dict.category != "All" else None
    )
...
def get_all_articles(filters):
    ...
    articles = frappe.db.get_all(
        "Blog", fields=fields, filters=filters, order_by="published_date desc"
    )

    for article in articles:
        article.member_doc = frappe.get_doc(
            "Team Member", article.published_by
        ).as_dict()
...
def get_all_frts_in_state(state):
    ...
    filters = {"published": 1, "state": state}
