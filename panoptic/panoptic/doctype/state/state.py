...
class State(WebsiteGenerator):
    ...
    published = 1
    website = frappe._dict(page_title_field="state_name", no_cache=1, sitemap=1)
    ...
    def get_context(self, context):
        ...
        if self.name == "Central":
            ...
            context.section_title = (
                "<span class='text-white'>FRT Systems in</span> {0}".format(
                    self.state_name
                )
            )
        ...
        context.total_frt = (
            frappe.db.count("FRT", {"state": self.name, "published": 1}) or 0
        )
        context.total_frt_in_use = (
            frappe.db.count(
                "FRT", {"state": self.name, "published": 1, "status": "In Utilization"}
            )
            or 0
        )
        context.total_cost = shorten_number(
            sum(
                frappe.db.get_all(
                    "FRT",
                    fields=["amount_spent"],
                    filters={"state": self.name, "published": 1},
                    pluck="amount_spent",
                )
            )
        )
    ...
    def get_all_cities(self, filters={}, fields=None):
        ...
        return frappe.db.get_all(
            "City", fields=fields, filters=filters, order_by="published_date desc"
        )
